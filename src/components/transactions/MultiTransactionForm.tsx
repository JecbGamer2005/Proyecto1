                    ...
                    <div className="flex space-x-2">
                      <input
                        ...
                        className={`flex-1 rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 ${
                          priceUpdateMode[index] ? 'bg-yellow-50 border-yellow-300' : ''
                        }`}
                        readOnly={!priceUpdateMode[index] && formData.type === 'entry'}
                      />
                      {formData.type === 'entry' && item.productId && (
                        <button
                          type="button"
                          ...
                          title={priceUpdateMode[index] ? 'Precio se actualizará en inventario' : 'Usar precio actual del inventario'}
                        >
                          {priceUpdateMode[index] ? '💰 Actualizar' : '🔒 Fijo'}
                        </button>
                      )}
                    </div>
                    {formData.type === 'entry' && item.productId && priceUpdateMode[index] && (
                      <p className="text-xs text-yellow-600 mt-1">
                        ⚠️ Este precio se guardará en el inventario (actual: {formatCurrency(getProductCurrentPrice(item.productId))})
                      </p>
                    )}
                  </div>
